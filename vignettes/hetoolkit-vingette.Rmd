...
Data validation, transformation (conversion) and predications are done within `predict_indices` using functionality predefined in the AquaMetrics RICT package (https://github.com/aquaMetrics/rict); specifically, the 'rict_predict' function is applied.
...
The `impute_flow` function is designed to infills missing records in daily flow time series for one or more sites (gauging stations) using either interpolation or an equipercentile  method. Imputation of missing flow data can improve the later estimation of flow statistics using the calc_flowstats() function and aid the visualisation of hydro-ecological relationships using the plot_hev() function.

The output dataset is in long format, similar to that produced by `import_flow`.  The output dataset contain an 'imputed' colunms; this is a flag  indicating whether each flow value is original (0) or imputed (1).

Additionally, this function plots up observed and imputed flow values for each site; enabling the user to validate the imputation.

```{r impute flow, warning=FALSE, message=FALSE, tidy=TRUE}

flow_data <- impute_flow(flow_data, 
...
model_full <- lme4::lmer(LIFE_F_OE ~ Q95z + Q10z + Q95zLS1 + (Q95z | biol_site_id), data = autumn_data, REML = FALSE)
...
# drop Q10z term
model_3 <- model_full <- lme4::lmer(LIFE_F_OE ~ Q95z + (Q95z | biol_site_id), data = autumn_data, REML = FALSE)
