...
pacman::p_load(dplyr, insight, lubridate, tictoc, readr, downloader, readxl, RCurl, writexl, tidyr, stringr, tibble, readr, htmlTable, tibbletime, devtools, roxygen2, downloader, shiny, plotly, RColorBrewer, ggnewscale, ggplot2, fasstr, lme4, sjmisc, RcppRoll, mgcv, viridis, gridExtra, naniar, purrr, ggfortify, plyr, GGally, forcats, visreg, formatR, sf, ggrepel, reshape, grid, glmmTMB, shiny)
...
download('https://environment.data.gov.uk/UserDownloads/interactive/844afa0ea3224fad8e32fcd33fa7f485187296/EA_AdminBoundEAandNEpublicFaceAreas_SHP_Full.zip', destfile = 'EA_AdminBoundEAandNEpublicFaceAreas_SHP_Full.zip')
...
Data validation, transformation (conversion) and predications are done within `predict_indices` using functionality predefined in the AquaMetrics RICT package (https://github.com/aquaMetrics/rict); specifically, the 'rict_predict' function from the aquaMetrics RICT package is applied.

The `predict_indices` function can accept environmental data formatted in either of two formats, specified using the 'file_format' agruement:
...
The code below demonstrates the how to summarise whether these columns contain NAs, and how to replace NAs as required. Note, this code will only run successfully if using data in the "EDE" file format.
...
If applying the 'equipercentile' method, an optional 'donor' arguement can be specified.  The 'donor' arguement must be specified in data frame format with at least two columns: the first a list of flow sites requiring imputation, and the second a list of paired donor sites. The function assumes that data for all flow sites specified within the 'donor' data table, including paired donor sites, are contained within the 'flow_data' supplied.  
...
donorsites <- unique(donor$donor_station)
...
flow_data_all <- flow_data %>% rbind(flow_data_donor)
...
This is an updated version of the `calc_flowstats` function.  The first iteration of the function remains available, using `calc_flowstats_old`.
...
# drop random slope
model_1 <- model_full <- lme4::lmer(LIFE_F_OE ~ Q95z + Q10z + Q95zLS1 + (1 | biol_site_id), data = autumn_data, REML = FALSE)
...
# drop Q95zLS1 term
model_2 <- model_full <- lme4::lmer(LIFE_F_OE ~ Q95z + Q10z + (Q95z | biol_site_id), data = autumn_data, REML = FALSE)
