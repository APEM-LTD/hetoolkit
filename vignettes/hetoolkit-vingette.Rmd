---
title: "HETOOLKIT Vingette"
output: 
  rmarkdown::html_vignette:
    toc: true
vignette: >
  %\VignetteIndexEntry{HETOOLKIT Vingette}
...
The `hetoolkit` package comprises a collection of functions for assembling, processing, visualising and modelling hydro-ecological data. This vignette illustrates a typical workflow using a selection of 20 macroinvertebrate sampling sites from the Environment Agency's National Drought Monitoring Network (NDMN).
...
* *biol_site_id* = id of macroinvertebrate sampling site.
...
master_data <- master_file %>% filter(biol_site_id %in% c("34310", "34343", "34352", "55287", "55395", "55417", "55673", "55824", "55897", "56065", "56226", "54637", "54769", "54801", "54962", "80998", "56491", "54827"))
...
A number of standardise column names are used throughout the `hetoolkit` package, and throughout this vingette and its associated datasets.  These include:
...
```{r bio, warning=FALSE, message=FALSE, tidy=TRUE, results='hide'}
...
```{r biol_drops, message=FALSE, warning=FALSE, results='hide', tidy=TRUE}
...
```{r env, warning=FALSE, message=FALSE, tidy=TRUE, results='hide'}
...
# download('https://environment.data.gov.uk/UserDownloads/interactive/d733# 68b35bb74163a384d3d29e479c0d75644/EA_AdminBoundEAandNEpublicFaceAreas_SH# P_Full.zip', destfile = # 'EA_AdminBoundEAandNEpublicFaceAreas_SHP_Full.zip')

## get Wales coastline, just because map looks silly without it
# download('https://borders.ukdataservice.ac.uk/ukborders/easy_download/pr# ebuilt/shape/Wales_ol_2001.zip', destfile = 'Wales_ol_2001.zip')

## load in the shapefiles to R objects and remove seaward parts
# EA.areas <- read_sf('/vsizip/EA_AdminBoundEAandNEpublicFaceAreas_SHP_Ful# l.zip/data/Administrative_Boundaries_Environment_Agency_and_Natural_Engl# and_Public_Face_Areas.shp')
...
```{r env_drops, warning=FALSE, message=FALSE, tidy=TRUE, results='hide'}
...
The `import_rhs` function allows the user to download RHS Data from Open Data.
...
```{r rhs, warning=FALSE, message=FALSE, tidy=TRUE, results='hide'}
...
```{r rhs_drops, warning=FALSE, message=FALSE, tidy=TRUE, results='hide'}
...
```{r join bio, warning=FALSE, message=FALSE, tidy=TRUE, results='hide'}
...
```{r join bio_2, warning=FALSE, message=FALSE, tidy=TRUE, results='hide'}
...
```{r OE ratios, warning=FALSE, message=FALSE, tidy=TRUE, results='hide'}
...
```{r import flow, warning=FALSE, message=FALSE, tidy=TRUE, results='hide'}
...
```{r calculate flow stats, warning=FALSE, message=FALSE, results='hide', tidy=TRUE}
...
```{r join RHS, warning=FALSE, message=FALSE, tidy=TRUE, results='hide'}
...
In this example, the first component (x-axis) can be interpreted a gradient in substrate composition from finer pebbles/gravels to coarser boulder/cobbles, and the second component (y-axis) can be interpreted as a longitudinal gradient from wider, deeper rivers (bottom) to smaller, steeper streams (top). On the basis of these results, we may wish to consider whether sites 77599 and 77216 are suitable for inclusion in the model, or whether they are sufficiently different to warrant being excluded.

## Assess coverage of hisotrical flow conditons

A hydro-ecological model has a greater chance of revealing relationships between biology metrics and flow variables of the biology samples span a wide range of flow conditions. Good coverage of historical flow conditions by the biology sample data also means that the calibrated model is likely to be better at predicting biological repsones under high and low flow scenarios.
...
Whilst the dataset outputted by the `join_he` function can be used to produce a HEV plot, but a better solution is achieved by manually joining the biology and flow datasets as described below. 
...
hev_data$Month <- month(hev_data$SAMPLE_DATE)
hev_data$Year <- year(hev_data$SAMPLE_DATE)
...
```{r hevplot2, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r cv2, message=FALSE, warning=FALSE, include=FALSE, results = 'hide'}
...
# RMSE higher than for model_full, so keep Q95z slope
cv_1[1]

```

```{r cv3, message=FALSE, warning=FALSE, include=FALSE, results = 'hide'}
...
# RMSE lower than for model_full, so drop Q95zLS1
cv_2[1]

```

```{r cv4, message=FALSE, warning=FALSE, include=FALSE, results = 'hide'}
...
# RMSE higher than for model_2, so keep Q10z
cv_3[1]

# no further model simplification possible
...
diag_plots <- diag_lmer(model = model_2, data = autumn_data, facet_by = NULL)
...
# generate diagnostic plots
diag_plots2 <- diag_lmer(model = model_2, data = autumn_data, facet_by = "biol_site_id", order_by = "mean", order = "descending", ncol = 4, scales = "fixed")
...
sjPlot::plot_model(model = model_2, type = "re", facet.grid = FALSE, free.scale = FALSE, title = NULL, vline.color = "darkgrey") + ylim(-0.2,0.2)
...
visreg::visreg(model_2, "Q95z", ylab = "LIFE_F_OE")
...
pi <- merTools::predictInterval(merMod = model_2, level = 0.95, n.sims = 1000,
