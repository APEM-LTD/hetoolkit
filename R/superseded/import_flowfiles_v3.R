...
import_flowfiles <- function(sites = NULL, dir, skip_num, col_order, date_format = "dmy", start_date = "1985-01-01", end_date = Sys.Date()){
  ...
  # stop if date_format is not a supported option
    if(!date_format %in% c("dmy", "dmy_h", "dmy_hm", "dmy_hms", "mdy", "mdy_h", "mdy_hm", "mdy_hms", "ymd", "ymd_h", "ymd_hm", "ymd_hms", "ydm", "ydm_h", "ydm_hm", "ydm_hms")){
      stop ("date_format is invalid; see ?import_flowfiles for supported date formats")
  }
  ...
  # dates are auto-formatted for xls and xlsx files; for csv, txt and all files, the dates are formatted according to the date-format argument
