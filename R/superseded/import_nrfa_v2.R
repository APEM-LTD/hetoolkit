...
#'
#' @usage
#' import_nfra(sites, start_date = "1985-01-01", end_data = Sys.Date()))
#'
...
#'
#' @details This function retrieves flow data from the online National River Flow Archive (NRFA; http://nrfa.ceh.ac.uk). The NRFA is the main archive of river flow data for the United Kingdom, and contains data from over 1500 gauging stations.
#'
...
#' import_nrfa(sites = c("1001", "2001"), start_date = "2000-01-01", end_date = "2019-12-31")
#'
...
import_nrfa <- function(sites, start_date = "1985-01-01", end_date = Sys.Date(), tidy = FALSE){
  ...
  flow_data <- purrr::map_dfr(sites, ~get_nrfa_data(.))
  ...
  # filter date by date
  flow_data <- flow_data %>%
    filter(flow_data$date >= start_date & flow_data$date <= end_date)
  ...
  #complete_data <- dplyr::left_join(full_grid, flow_data, by = c("flow_site_ID" = "Station", "date"))
