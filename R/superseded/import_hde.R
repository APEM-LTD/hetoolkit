...
import_hde <- function(sites,
  ...
  #check if any of the defined stations are present in the HDE station list
  if(length(stations_hde) == 0 ) {
      #stop if none of the user defined sites are present in the HDE station list
    stop("Cant find any matching HDE stations")
  } else if(length(stations_cant_find) > 0 ){
        warning(print(paste('Warning, could not find these stations: ', stations_cant_find)))
  }
