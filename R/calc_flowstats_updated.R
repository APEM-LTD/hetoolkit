...
calc_flowstats <- function(data,
  ...
  if(is.null(date_range) == FALSE && IsDate(date_range[1], "%Y-%m-%d") == FALSE){""} else {
    if(is.null(date_range) == FALSE && IsDate(date_range[2], "%Y-%m-%d") == FALSE){""}
  ...
  if(is.null(date_range) == FALSE && IsDate(date_range[1], "%Y-%m-%d") == TRUE && IisTRUE(date_range[1] > Sys.Date()) == TRUE)
  {stop("date_range[1] is in the future")}
  if(is.null(date_range) == FALSE && IsDate(date_range[2], "%Y-%m-%d") == TRUE && IisTRUE(date_range[2] > Sys.Date()) == TRUE)
  ...
  if(is.null(date_range) == FALSE){
    data_1 <- data_1 %>% dplyr::filter(date <= date_range[2] & date >= data_range[1])
  }
  ...
  df1.3 <- df1.2 %>%
    dplyr::select(-Q5mean, -Q5sd, -Q10mean, -Q10sd, -Q20mean, -Q20sd,
                  ...
                  -Q70mean, -Q70sd, -Q75mean, -Q75sd, -Q80mean, -Q80sd,
  ...
  # re-order cols
  col_order <- c("site", "win_no", "start_date", "end_date", "n_data", "n_missing", "n_total", "prop_missing", "mean", "sd", "lsd", "Q5", "Q10", "Q20", "Q25", "Q30", "Q50", "Q70", "Q75", "Q80", "Q90", "Q99", "Q5z", "Q10z", "Q20z", "Q25z", "Q30z", "Q50z", "Q70z", "Q75z", "Q80z", "Q90z", "Q99z", "dry_n", "dry_e", "dry_start", "dry_end", "dry_mid", "low_n", "low_e", "low_start", "low_end", "low_mid", "low_magnitude", "low_severity", "high_n", "high_e", "high_start", "high_end", "high_mid", "vol", "vol_z", "min", "min_z", "min_doy", "min_7day", "min_7day_z", "min_7day_doy", "min_30day", "min_30day_z", "min_30day_doy", "max", "max_z", "max_doy")
  df1 <- df1.3[, col_order]
  }
  ...
  # re-order cols
  col_order <- c("site", "win_no", "start_date", "end_date", "n_data", "n_missing", "n_total", "prop_missing", "n_imputed", "prop_imputed", "mean", "sd", "lsd", "Q5", "Q10", "Q20", "Q25", "Q30", "Q50", "Q70", "Q75", "Q80", "Q90", "Q99", "Q5z", "Q10z", "Q20z", "Q25z", "Q30z", "Q50z", "Q70z", "Q75z", "Q80z", "Q90z", "Q99z", "dry_n", "dry_e", "dry_start", "dry_end", "dry_mid", "low_n", "low_e", "low_start", "low_end", "low_mid", "low_magnitude", "low_severity", "high_n", "high_e", "high_start", "high_end", "high_mid", "vol", "vol_z", "min", "min_z", "min_doy", "min_7day", "min_7day_z", "min_7day_doy", "min_30day", "min_30day_z", "min_30day_doy", "max", "max_z", "max_doy")
  df1 <- df1.4[, col_order]
  ...
  if(is.null(ref_col) == FALSE){
    ...
    df1_ALL$vol_z_ref <- (vol - volmean_ref) / vol_sd_ref
    ...
    # Get min_day7_z_adj
    min_day7 <- df1_ALL$min_day7
    min_day7mean_ref <- df1_ALL$min_day7mean_ref
    min_day7sd_ref <- df1_ALL$min_day7sd_ref
    df1_ALL$min_day7_z_ref <- (min_day7 - min_day7mean_ref) / min_day7sd_ref

    # Get min_day30_z_adj
    min_day30 <- df1_ALL$min_day30
    min_day30mean_ref <- df1_ALL$min_day30mean_ref
    min_day30sd_ref <- df1_ALL$min_day30sd_ref
    df1_ALL$min_day30_z_ref <- (min_day30 - min_day30mean_ref) / min_day30sd_ref
