...
calc_flowstats <- function(data,
  ...
  long_data <- CreateLongData(my_data_2, QSTATS1)
  ...
  # add min & max dates to long_data, for df2 of final function output
...
CalcFlowStats <- function (group1, group2, flowts) {
  ...
  MISSING <- flow_data %>% dplyr::group_by(site, win_no) %>%
  ...
  flowts<- flow_data %>%  dplyr::group_by(site, win_no) %>%
